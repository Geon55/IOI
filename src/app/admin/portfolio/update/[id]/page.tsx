...
export default function UpdatePortfolioPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = use(params);
    ...
    const fetchPortfolio = async () => {
        ...
        if (data && !error) {
            let fetchedBlocks = Array.isArray(data.content_blocks) ? data.content_blocks : [];
            ...
            let formMakingProcess = '';

            const metadataIndex = fetchedBlocks.findIndex((b: any) => b.type === 'metadata');
            if (metadataIndex !== -1) {
                try {
                    const extraData = JSON.parse(fetchedBlocks[metadataIndex].content);
                    formClient = extraData.client || '';
                    formLocation = extraData.location || '';
                    formDate = extraData.date || '';
                    formMoreInfo = extraData.more_info || '';
                    initialHeroImage = extraData.hero_image || '';
                    formMakingProcess = extraData.making_process || '';
                } catch (e) { }
                fetchedBlocks = fetchedBlocks.filter((_: any, idx: number) => idx !== metadataIndex);
            }

            // Client-side IDs for blocks
            const blocksWithIds = fetchedBlocks.map((b: any) => ({
                ...b,
                id: Math.random().toString(36).substring(7),
                previewUrl: b.type === 'image' ? b.content : undefined
            }));

            const initialForm = {
                title: data.title || '',
                eng_title: data.eng_title || '',
                category: data.category || '조형물',
                client: formClient,
                ...
                more_info: formMoreInfo,
                making_process: formMakingProcess
            };

            setForm(initialForm);

            // Automatically open toolbar fields if they have data
            setActiveFields(prev => ({
                ...prev,
                category: true, // Always show category by default for convenience
                eng_title: !!initialForm.eng_title,
                client: !!initialForm.client,
                date: !!initialForm.date,
                creative_director: !!initialForm.creative_director,
                design: !!initialForm.design,
                summary: !!initialForm.summary,
                more_info: !!initialForm.more_info,
                making_process: !!initialForm.making_process,
            }));
    ...
    // Image Handlers removed - inline upload directly through content blocks used.
    ...
    const handleReCropBlock = (id: string) => {
        ...
        if (block?.originalFile) {
            ...
            setCropModalInfo({ file: block.file, aspect: parseAspectRatio(block.aspectRatio || 'auto'), target: id });
        } else {
            alert('초기 등록된 사진은 원본 정보가 없어 크롭을 재조정할 수 없습니다. 새로 업로드해주세요.');
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            let mainImageUrl = imagePreview; // Fallback to existing url
            if (mainImage) {
            ...
            // Hero Image direct upload removed
            ...
            const processedBlocks = await Promise.all(blocks.map(async (block) => {
                ...
                return { type: block.type, content: block.content, aspectRatio: block.aspectRatio }; // keep existing url or text
            }));
            ...
            const metadataBlock = {
                ...
                content: JSON.stringify({
                    hero_image: heroImageUrl, // Automatically fallback to first body image if missing
                    client: form.client,
    ...
    if (isFetching) {
        return <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>불러오는 중...</div>;
    }
    ...
    return (
        <div style={{ maxWidth: '900px', margin: '0 auto', paddingBottom: '4rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
                ...
                <h1 style={{ fontSize: '1.875rem', fontWeight: 'bold', color: '#111827', margin: 0 }}>포트폴리오 수정 (상세형)</h1>
            </div>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                ...
                <div style={{ backgroundColor: 'white', padding: '2rem', borderRadius: '0.5rem', boxShadow: '0 1px 3px 0 rgba(0,0,0,0.1)' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.5rem 1rem', backgroundColor: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '0.5rem', marginBottom: '2rem' }}>
                        <div>
                            ...
                            {!imagePreview && <p style={{ fontSize: '0.8rem', color: '#ef4444', margin: '4px 0 0 0', fontWeight: 'bold' }}>⚠️ 저장하려면 썸네일이 필요합니다.</p>}
                        </div>
                        {imagePreview && (
                                                    ...
                                                    {(block.file || block.previewUrl) && (
                                                        <button
                                                            ...
                                                            onClick={(e) => {
                                                                e.preventDefault();
                                                                if (block.originalFile) {
                                                                    setCropModalInfo({ file: block.originalFile, aspect: 3 / 4, target: 'main' });
                                                                } else {
                                                                    alert('기존 등록 이미지는 여기서 직접 자를 수 없습니다. 새 이미지를 추가하여 썸네일로 지정하세요.');
                                                                }
                                                            }}
                ...
                <div style={{ position: 'sticky', bottom: '2rem', display: 'flex', justifyContent: 'flex-end', marginTop: '1rem' }}>
                    <button
                        ...
                        style={{
                            backgroundColor: isLoading || !imagePreview ? '#9ca3af' : '#2563eb',
                            color: 'white',
                            padding: '1rem 3rem',
                            borderRadius: '0.5rem',
                            border: 'none',
                            fontWeight: 'bold',
                            fontSize: '1.1rem',
                            cursor: isLoading || !imagePreview ? 'not-allowed' : 'pointer',
                            boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)'
                        }}
                        title={!imagePreview ? "썸네일이 필요합니다" : ""}
                    >
                        {isLoading ? '수정 처리 중...' : '포트폴리오 수정하기'}
                    </button>
                </div>
